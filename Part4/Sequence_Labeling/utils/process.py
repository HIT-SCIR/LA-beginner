...
class Processor(object):
    ...
    def train(self):
        ...
        best_epoch = 0
        no_improve = 0
        dataloader = self.__dataset.get_data_loader('train')
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch in tqdm(dataloader, ncols=50):
                padded_text, sorted_slot, seq_lens = self.__dataset.add_padding(
                    text_batch, slot_batch)
                flat_sorted_slot = list(Evaluator.expand_list(sorted_slot))

                text_var = torch.LongTensor(padded_text)
                slot_var = torch.LongTensor(flat_sorted_slot)
    ...
    def prediction(model, dataset, mode, batch_size, args):
        ...
        with torch.no_grad():
            for text_batch, slot_batch in tqdm(dataloader, ncols=50):
                padded_text, sorted_slot, seq_lens = dataset.add_padding(
                    text_batch, slot_batch, digital=False
                )
                ...
                slot_out = model(var_text, seq_lens)

...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        num = min(len(unfinished_parses), batch_size)
        input = unfinished_parses[:num].copy()
        transitions = model.predict(input)
        for i in range(num):
            input[i].parse_step(transitions[i])
            if len(input[i].stack) == 1 and len(input[i].buffer) == 0:
                unfinished_parses.remove(input[i])

...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    u_matrix = outsideVectors
    y = np.zeros((len(outsideVectors), 1))
    y[outsideWordIdx] = 1
    y_hat = np.matrix(softmax(outsideVectors @ centerWordVec)).T
    v_c = np.matrix(centerWordVec).T

    gradCenterVec = u_matrix.T @ (y_hat - y)
...
def negSamplingLossAndGradient(
    ...
    for idx in negSampleWordIndices:
        gradCenterVec -= (sigmoid(-u_matrix[idx] @ v_c) - 1) * u_matrix[idx]

    gradOutsideVecs = np.zeros_like(u_matrix)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(u_o @ v_c) - 1) * v_c
    for idx in negSampleWordIndices:
        gradOutsideVecs[idx] = -negSampleWordIndices.count(
            idx) * (sigmoid(-u_matrix[idx] @ v_c) - 1) * v_c

    loss = -np.log(sigmoid(u_o @ v_c))
    for idx in negSampleWordIndices:
        loss -= np.log(sigmoid(- u_matrix[idx] @ v_c))
    # END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        ...
        current_loss, current_gradCenterVec, current_gradOutsideVectors = word2vecLossAndGradient(
            centerWordVec=center_word_vec, outsideWordIdx=outside_word_idx, outsideVectors=outsideVectors, dataset=dataset)
        current_gradCenterVec = np.ravel(current_gradCenterVec)

        loss += current_loss
        gradCenterVecs[center_word_idx] += current_gradCenterVec

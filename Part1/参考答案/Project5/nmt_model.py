...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.charDecoder is not None:
            ...
            target_chars = target_padded_chars[1:].contiguous().view(-1, max_word_len) #(max_sentence_length, batch_size, max_word_length)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings_source(source_padded) #获取源句子词嵌入
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(target_padded)  #target sentence
        for Y_t in torch.split(Y,1): 
            ...
            combined_outputs.append(o_t) #保存每个时间步的最终输出（hidden与encoder每个时间步加权 拼接）用于最后预测单词 (batch_size,h)
            o_prev = o_t #
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.cat((dec_state[0],a_t),dim = 1)    #会影响结果吗
